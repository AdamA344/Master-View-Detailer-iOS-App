...
class taskProgressView: UIView {
    ...
    fileprivate func createCircularPath() {
        ...
        let circlePath = UIBezierPath(arcCenter: CGPoint(x: frame.size.width/4, y: frame.size.height/2), radius: (frame.size.width - 1.5)/4, startAngle: CGFloat(-0.5 * .pi), endAngle: CGFloat(1.5 * .pi), clockwise: true)
